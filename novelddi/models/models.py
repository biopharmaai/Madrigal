...
from novelddi.chemcpa.chemCPA.model import TxAdaptingComPert
...
def get_tx_encoder(tx_encoder_name, tx_encoder_hparams, embed_dim, use_modality_pretrain=True):
    if tx_encoder_name == 'chemcpa':
        ...
        # FIXME: Parameterize those hard-coded stuff
        ...
        if not use_modality_pretrain:
            ...
            for key in list(state_dict.keys()):
                ...
                if key.startswith("adversary_") or key == "drug_embeddings.weight":
                    state_dict.pop(key)
            # assert (np.array(model_config["covariate_names_unique"]["cell_iname"]) == np.array(CELL_LINES_CAPITALIZED)).all()  # NOTE: can actually be re-initialized or truncated, but just for the sake of simplicity, we adapt the encoder with exactly the same cell lines
...
class TransformerFusion(nn.Module):
    def __init__(self, embed_dim, num_tx_bottlenecks, transformer_num_layers, transformer_att_heads, transformer_head_dim, transformer_ffn_dim, transformer_dropout = 0.1, transformer_actn = 'relu', transformer_norm_first = False, transformer_batch_first = True, transformer_agg='mean'):
        ...
        if self.transformer_agg == 'x-attn':
            ...
            if num_tx_bottlenecks > 0:
                self.x_attn_key_padding_mask[:, :NUM_NON_TX_MODALITIES] = True  # NOTE: mask out the non-TX modalities
                self.x_attn_key_padding_mask[:, -len(CELL_LINES):] = True  # NOTE: mask out the TX modalities
    ...
    def forward(self, fusion_sequence, fusion_mask, src_mask=None):
        ...
        #     src_mask = src_mask.unsqueeze(0).repeat(batch_size, 1, 1)  # NOTE: no need to repeat the mask across batch. if really needed, we will need to construct N * num_heads masks.
        ...
        fusion_sequence = self.transformer_encoder(src = fusion_sequence, src_key_padding_mask = fusion_mask, mask = src_mask)  # NOTE: `mask` in argument == src_mask
        ...
        if self.transformer_agg == 'cls':
            ...
            )[0]  # NOTE: returns [1, batch_size, feat_dim]
...
class BilinearDDIScorer(nn.Bilinear):
    def __init__(self, input_dim1: int, input_dim2: int, output_dim: int):
        """
        Extend torch.nn.Bilinear to accommodate our need of outputting a tensor of shape (num_label, input_n1, input_n2) 
        (note that for binary, num_label=1).
        ...
        # self.bias = nn.Parameter(self.bias.unsqueeze(-1).unsqueeze(-1))

    # def bilinear(self, input1: torch.Tensor, input2: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor):
    def bilinear(self, input1: torch.Tensor, input2: torch.Tensor, weight: torch.Tensor):
        # NOTE: weight is [num_labels, dim1, dim2]. since broadcasting applies to the first dim, we don't need to worry about the broadcasting of inputs across labels (see https://pytorch.org/docs/stable/generated/torch.matmul.html)
        # return torch.matmul(torch.matmul(input1, weight), input2.T) + bias
    ...
    def forward(self, input1: torch.Tensor, input2: torch.Tensor, label_range: tuple = None):
        if label_range is not None:
            ...
            # out = self.bilinear(input1, input2, self.weight[label_range[0]:label_range[1], :, :], self.bias[label_range[0]:label_range[1], :, :])
            out = self.bilinear(input1, input2, self.weight[label_range[0]:label_range[1], :, :])
        else:
            # out = self.bilinear(input1, input2, self.weight, self.bias)
...
# TODO: Implement this (requires changes also to the implementation inside nn.MultiheadAttention)
class PositionEncodingRotary(nn.Module):
    pass
...
class NovelDDIEncoder(nn.Module):
    def __init__(self, all_kg_data, feat_dim, str_encoder_name, str_encoder_hparams, kg_encoder_name, kg_encoder_hparams, cv_encoder_name, cv_encoder_hparams, tx_encoder_name, tx_encoder_hparams, num_tx_bottlenecks, pos_emb_dropout, transformer_fusion_hparams, proj_hparams, fusion='transformer_uni_proj', str_node_feat_dim=MOL_DIM, use_modality_pretrain=True, normalize=False, pos_emb_type='learnable', adapt_before_fusion=False, **kwargs):
        ...
        if self.num_tx_bottlenecks == 0:
            ...
            # TODO: Experiment - only adding pos enc to the non-TX modalities 
            ...
            if self.fusion in {'transformer_uni_proj', 'transformer'}:
                ...
                if self.transformer_agg == 'cls':
                    ...
                    if self.num_tx_bottlenecks > 0:
                        ...
                        # TODO: Even when we use MAX aggregation, there must be some placeholder [CLS] token that attends to all, otherwise, some tokens could be nan
