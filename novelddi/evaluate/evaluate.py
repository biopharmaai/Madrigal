...
from novelddi.evaluate.GeomCA import GeomCA
from novelddi.evaluate.metrics import get_metrics
from novelddi.utils import from_indices_to_tensor, to_device, NUM_MODALITIES
from novelddi.evaluate.eval_utils import (
    AVERAGE, 
    KEY_METRIC_DICT, 
    NUMBER2MODALITY, 
...
###
# Pretraining evaluation (inst dist acc; loss; uniformity)
###
@torch.no_grad()
def evaluate_pt(model, drugs, masks, too_hard_neg_mask, collator, split, wandb, logger, device, epoch):
    ...
    for comp_pair, metrics in zip([
        ...
        # 'kg v tx_mcf7', 'kg v tx_pc3', 'kg v tx_vcap', 
        # 'cv v tx_mcf7', 'cv v tx_pc3', 'cv v tx_vcap'
    ], [
        ...
        # kg_tx_mcf7_ret_metrics, kg_tx_pc3_ret_metrics, kg_tx_vcap_ret_metrics,
    ]):
    ...
    all_embeds = {}
    for mod in [0,1,2,13,15,17]:  # NOTE: str, kg, cv, mcf7, pc3, vcap
    ...
    for mod1, mod2 in [(0, 1), (0, 2), (1, 2), (0, 13), (0, 15), (0, 17)]:  # TODO: record alignment between more combinations
...
def get_inst_dist_topk_accuracy(embeds1, embeds2, k: int, metric: str = 'cosine'):
    ...
    """
    if metric=='euclidean':
        sim_mat = distance_matrix(embeds1, embeds2, p=2)  # p is minkowski's p
        ...
        embeds2 = embeds2 / torch.norm(embeds2, dim=1, keepdim=True)  # L2-norm for cosine similarity
...
# TODO: uniformity viz; cross-modality prediction
###
